...
internal sealed class DiseaseRepository(
    WfDbContext context,
    EncryptionHelper encryptionHelper)
    : BaseRepository<Disease>(context), IDiseaseRepository
{
    private readonly WfDbContext _context = context;

    public async Task<IEnumerable<Disease>> GetListAsync(int take, int skip, int? doctorId = null)
    {
        var query = _context.Diseases.Where(d => d.IsActive);
    ...
    public async Task<Disease?> GetDetailsAsync(int id)
    {
    ...
    public async Task<IEnumerable<Disease>> GetActiveByDoctorIdAsync(int doctorId)
    {
        return await _context.Diseases
